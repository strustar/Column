...
def Cal(In, Reinforcement_Type):
	# Input Data
    RC_Code = In.RC_Code;  FRP_Code = In.FRP_Code;  Column_Type = In.Column_Type;  Section_Type = In.Section_Type;  PM_Type = In.PM_Type
    ...
    [nst, ni, Ast] = [np.zeros(Layer), np.zeros(Layer), [np.pi*d**2/4 for d in dia]]
    ...
    Pn[z1] = -np.sum(Ast)*fy/1e3;  ee[z1] = -0.000000001 #Mn[z1]/Pn[z1]*1e3
    ...
    if 'FRP' in Reinforcement_Type and 'ACI 440.1' in FRP_Code:    #! for ACI 440.1R**  Only Only   
        if 'Rectangle' in Section_Type: M = (2*gamma - 1)**2/(2*gamma) *(A1 + A2/3)*f_fu*hD/1e6
        if 'Circle' in Section_Type:    M = (2*gamma - 1)**2/(8*gamma) *np.sum(Ast)*f_fu*hD/1e6
...
###* Calculation Point C(3-1), D(4-1) (Zero Tension, Balacne Point) and E(5-1) &&  Z = eps/ep_cu  0.1~9.?
    ...
    for zz in [1, 2]:
        ...
        for z1 in range(zz1, zz2+1):            
            if ('FRP' in Reinforcement_Type) and ('ACI 440.1' in FRP_Code):             #############! for ACI 440.1**
                ...
                c = d*ep_cu/(ep_cu + eps)
                if 'Rectangle' in Section_Type:  bhD = [hD, be, height]
                if 'Circle'    in Section_Type:  bhD = [hD, D]
    ...
    for z1 in [2-1, 6-1]:        
        ...
        if ('FRP' in Reinforcement_Type) and ('ACI 440.1' in FRP_Code):             #############! for ACI 440.1**            
            if z1 == 2-1:
                if 'Rectangle' in Section_Type:
                    x = alpha/beta1*height;  alp = x/d;   c = x
                    Cc = Pnn;           Lc = (1 - alp*beta1*gamma)*height/2;     M = Cc*Lc/1e3
                if 'Circle' in Section_Type:
                    for k3 in [1, 2, 3]:
    ...
    for zz in [1, 2]:
        if zz == 1:
            n = len(Pn);  phi = np.zeros(n);  phi_Status = ['' for _ in range(n)]
        if zz == 2:
        ...
        for z1 in range(n-1):
            ...
            if 'RC' in Reinforcement_Type:       #! RC : KDS-2021 & KCI-2012
                ...
                ph = phi0 + (eps - ep_tccl)*(0.85 - phi0)/(ep_ttcl - ep_tccl);  ph_Status = 'Transition Zone'
                if eps <= ep_tccl: ph = phi0;  ph_Status = 'Compression Controlled'
                if eps >= ep_ttcl: ph = 0.85;  ph_Status = 'Tension Controlled'
            ...
            if 'FRP' in Reinforcement_Type:      #! FRP : AASHTO & ACI  (ACI 440.1R**은 약간 다른데 거의 비슷해서 여기에 포함)
                ...
                ph = a1 - a2*eps/ep_fu;  ph_Status = 'Transition Zone'
                if eps <= 0.8*ep_fu: ph = a3;  ph_Status = 'Compression Controlled'
                if eps >= ep_fu: ph = 0.55;    ph_Status = 'Tension Controlled'
            if zz == 1: phi[z1] = ph;  phi_Status[z1] = ph_Status
            if zz == 2: Zphi[z1] = ph    
    if 'RC'  in Reinforcement_Type:  [phi[-1], Zphi[-1]] = [0.85, 0.85];  phi_Status[-1] = 'Tension Controlled'
    if 'FRP' in Reinforcement_Type:  [phi[-1], Zphi[-1]] = [0.55, 0.55];  phi_Status[-1] = 'Tension Controlled'
    ### 강도감소계수 (phi)
    Pd = phi*Pn;  Md = phi*Mn;  ZPd = Zphi*ZPn;  ZMd = Zphi*ZMn

    PM = PM1()
    In.hD = hD;  In.nhD = nh
    if 'Circle' in Section_Type:  In.nhD = nD
    ...
    PM.Ag = Ag;  PM.Ast = Ast;  PM.nst = nst;  PM.dsi = dsi;  PM.rho = rho
    
    PM.e = ee;  PM.c = cc;  PM.Pn = Pn;  PM.Mn = Mn;  PM.ep_s = ep_s;  PM.fs = fs
    PM.phi_Status = phi_Status;  PM.phi = phi;  PM.Pd = Pd;  PM.Md = Md
