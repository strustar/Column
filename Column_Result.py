...
size8 = 8;  size15 = 15;  size17 = 17;  size20 = 20
...
def Fig(In, R, F):
    ...
    with PM_RC:   container_PM_RC  = st.container()
    with PM_FRP:  container_PM_FRP = st.container()
    
    column_RC, column_Common, column_FRP = st.columns([1., 1, 1.], gap = "small")
    with column_Common: container_column_Common  = st.container()
    with column_RC:     container_column_RC      = st.container()
    with column_FRP:    container_column_FRP     = st.container()

    table_RC, table_Common, table_FRP = st.columns([1., 1, 1.], gap = "small")
    with table_Common:
    ...
    fig = PM_plot(In, R, F, 'FRP', selected_row);  container_PM_FRP.plotly_chart(fig, config={'displayModeBar': False})

    # selected_row가 설정된 상태에서 기둥을 그리기 위해서
    fig = Column(In, R, F, 'Common', None);       container_column_Common.plotly_chart(fig,  config={'displayModeBar': False})
    fig = Column(In, R, F, 'RC'  ,selected_row);  container_column_RC.plotly_chart(fig,  config={'displayModeBar': False})    
    fig = Column(In, R, F, 'FRP' ,selected_row);  container_column_FRP.plotly_chart(fig, config={'displayModeBar': False})    
...
def Table(In, R, F, loc, selected_row):
    ...
    columnwidth = [1, 1, 1, 1, 1, 1, 1];  height = 338;  width = 550
    left = 1;  cells_align = 'right';  n = 100
    if selected_row != None:
        if 'A (P' in selected_row: n = 0
        if 'B (M' in selected_row: n = 1
        if 'C (Z' in selected_row: n = 2
        if 'D (B' in selected_row: n = 3
        if 'E '   in selected_row: n = 4
        if 'F (P' in selected_row: n = 5
        if 'G (P' in selected_row: n = 6
    ...
    if 'Full Data' in loc:
        ...
        columnwidth = [1, 0.9, 1, 0.8, 0.8, 1, 0.8,1,1,1,1];  height = 3900;  width = 840        
...
def red_text(data):
    ...
    sz = data.shape;  sgn = np.zeros([sz[0], sz[1]])
    for i in range(sz[0]):
        for j in range(sz[1]):
            if data[i][j] < 0:  sgn[i, j] = 1
...
def common_table(headers, data, columnwidth, cells_align, cells_fill_color, height, left, **kargs):
    ...
    fill_color = ['gainsboro'];  lw = 2    
    if len(kargs) > 0:  fill_color = kargs['fill_color'];  color = kargs['color'];  width = kargs['width']
...
def dimension(fig, x0,y0,length, fillcolor, color, width, txt, loc, opt):
    ...
    if 'False' not in opt:  annotation(fig, tx,ty, color, txt, locx, locy)  # 치수 문자
    ...
    for i in [1, 2]:
        ...
        fig.add_shape(    # 화살표
            type="path", path=f"M {a0} {b0} L {a1} {b1} L {a2} {b2} Z", # M = move to, L = line to, Z = close path
            fillcolor=fillcolor, line=dict(color=color, width=width, ), )


def PM_plot(In, R, F, loc, selected_row):
    fig = go.Figure();  fig_width = 800;  fig_height = 620
    ...
    if 'RC' in In.PM_Type:
        ...
        if 'RC' in loc:
            title = titleRC;  color1 = 'red';  dash1 = 'dot';  color2 = 'green';  dash2 = 'solid'
            PM_x1 = R.ZMn;  PM_y1 = R.ZPn;  PM_x2 = R.ZMd;  PM_y2 = R.ZPd;  PM_x7 = R.Mn;  PM_y7 = R.Pn;  PM_x8 = R.Md;  PM_y8 = R.Pd    
        if 'FRP' in loc:
            title = titleFRP;  color1 = 'magenta';  dash1 = 'dot';  color2 = 'royalblue';  dash2 = 'solid'
    ...
    if 'RC' not in In.PM_Type:
        ...
        if 'RC' in loc:
            title = titlePM;  color1 = 'red';  dash1 = 'dot';  color2 = 'magenta';  dash2 = 'dot'
            xtxt = 'M<sub>n</sub> [kN·m]';  ytxt = 'P<sub>n</sub> [kN]'
            PM_x1 = R.ZMn;  PM_y1 = R.ZPn;  PM_x2 = F.ZMn;  PM_y2 = F.ZPn;  PM_x7 = R.Mn;  PM_y7 = R.Pn;  PM_x8 = F.Mn;  PM_y8 = F.Pn
        if 'FRP' in loc:
            title = titlephiPM;  color1 = 'green';  dash1 = 'solid';  color2 = 'royalblue';  dash2 = 'solid'
            xtxt = 'ϕM<sub>n</sub> [kN·m]';  ytxt = 'ϕP<sub>n</sub> [kN]'
            PM_x1 = R.ZMd;  PM_y1 = R.ZPd;  PM_x2 = F.ZMd;  PM_y2 = F.ZPd;  PM_x7 = R.Md;  PM_y7 = R.Pd;  PM_x8 = F.Md;  PM_y8 = F.Pd        

    fig.add_trace(go.Scatter(x=PM_x1, y=PM_y1, mode='lines', name=legend1, line = dict(width=3, color=color1, dash=dash1, shape='spline')))
    fig.add_trace(go.Scatter(x=PM_x2, y=PM_y2, mode='lines', name=legend2, line = dict(width=3, color=color2, dash=dash2, shape='spline')))

    xmax = 1.19*max(PM_x1)
    ymin = 1.25*min([min(PM_y1), min(PM_y2)]);  ymax = 1.15*max(PM_y1)
    shape(fig, 'rect', 0,ymin,xmax,ymax, 'rgba(0,0,0,0)', 'RoyalBlue', 3)  # 그림 상자 외부 박스
    
    # Update the layout properties
    fig.update_layout(
        autosize=False, width=fig_width, height=fig_height, margin=dict(l=0, r=0, t=0, b=0), #hovermode='x unified',
        ...
        annotations=[dict(
            text=title, x=xmax/2, y=0.99*(ymax-ymin)+ymin, xanchor='center', yanchor='top', bgcolor='gold', showarrow=False,
            font=dict(size=size20, family=family, color='black'), )], )
    fig.update_traces(hovertemplate='M : %{x}<br>P : %{y}')
    fig.update_xaxes(
        range=[0, xmax], showgrid=True, gridwidth=2, #gridcolor='gray', griddash='solid',
        showspikes=True, spikecolor="gray", spikesnap="cursor", spikemode="across", spikethickness=3,
        title = dict(text=xtxt, standoff=20,
                font = dict(size=size17, family=family, color='black')),
        tickfont = dict(size=size17, color='black'), tickformat=',.0f', nticks=10, tickmode='auto', )
    fig.update_yaxes(
        range=[ymin, ymax], showgrid=True, gridwidth=2, #gridcolor='gray', griddash='solid',
        showspikes=True, spikecolor="gray", spikesnap="cursor", spikemode="across", spikethickness=3,
        title = dict(text=ytxt, standoff=20,
                font = dict(size=size17, family=family, color='black')),
        tickfont = dict(size=size17, color='black'), tickformat=',.0f', nticks=10, tickmode='auto',
    ...
    if 'RC' in In.PM_Type:   #! PM_Type = RC vs. FRP 일때만 Only
        # phi*Pn(max)
        if 'RC'  in loc:  [x, y, color] = [R.Md[2-1], R.Pd[2-1], 'green']
        if 'FRP' in loc:  [x, y, color] = [F.Md[2-1], F.Pd[2-1], 'royalblue']
        ...
        for i in [1, 2, 3]:
            if 'RC'  in loc:  [x, y] = [R.Mn[i], R.Pn[i]]
            if 'FRP' in loc:  [x, y] = [F.Mn[i], F.Pn[i]]            
            shape(fig, 'line', 0,0,x,y, None, 'black', 1)

            if 'RC'  in loc:  val = R.e[i]
            if 'FRP' in loc:  val = F.e[i]            
            f = 0.45 if i == 1 else 0.35
            color = 'blue' if val > 0 else 'red'
            if i == 1:  txt = f'e<sub>min</sub> = {val:,.1f} mm'
            if i == 2:  txt = f'e<sub>0</sub> = {val:,.1f} mm'
            if i == 3:  txt = f'e<sub>b</sub> = {val:,.1f} mm'
    ...
    for i in [1, 2]:     # 1 : Pn-Mn,  2 : Pd-Md
        for z1 in range(len(PM_x7)):
            if i == 1:
                ...
                if z1 == 1-1: txt = 'A (Pure Compression)'   # \;, \quad : 공백 넣기
                if z1 == 2-1: txt = 'B (e<sub>min</sub>)'
                if z1 == 3-1: txt = 'C (e<sub>0</sub>) Zero Tension'
                if z1 == 4-1: txt = 'D (e<sub>b</sub>)          <br>Balance      <br>Point          '
                if z1 == 5-1: txt = 'E (ε<sub>t</sub> = 2.5ε<sub>y</sub>)'
                if z1 == 6-1: txt = 'F (Pure Moment)'
                if z1 == 7-1: txt = 'G (Pure Tension)'

                if 'RC'  in loc:  bgcolor = 'lightgreen' if z1 == 4-1 else None
                if 'FRP' in loc:  bgcolor = 'lightblue'  if z1 == 4-1 else None
                [sgnx, xanchor] = [1, 'left']                
                sgny = -1 if z1 >= 4-1 else 1
                if 'FRP' in loc:
                    if z1 == 5-1: [sgnx, xanchor, txt] = [-1, 'right', 'E (ε<sub>t</sub> = 0.8ε<sub>fu</sub>)'                    ]
                    if z1 == 6-1: txt = 'F'
                if 'RC' not in In.PM_Type:
                    if z1 == 5-1: txt = 'E'
                    if z1 == 4-1 and 'FRP' in loc: [sgny, txt] = [1, 'D']
            ...
            if z1 == len(PM_x7) -1:  color = 'black'
            if z1 == 1-1: color = 'red'
            if z1 == 2-1: color = 'green'
            if z1 == 3-1: color = 'blue'
            if z1 == 4-1: color = 'cyan'
            if z1 == 5-1: color = 'magenta'
            if z1 == 6-1: color = 'yellow'
            if z1 == 7-1: color = 'darkred'
    ...
    if selected_row != None:
        if 'A (P' in selected_row: [n, txt] = [0, 'A (Pure Compression)']
        if 'B (M' in selected_row: [n, txt] = [1, 'B (Minimum Eccentricity)']
        if 'C (Z' in selected_row: [n, txt] = [2, 'C (Zero Tension)']
        if 'D (B' in selected_row: [n, txt] = [3, 'D (Balance Point)']
        if 'E ' in selected_row: n = 4
        if 'F (P' in selected_row: [n, txt] = [5, 'F (Pure Moment)']
        if 'G (P' in selected_row: [n, txt] = [6, 'G (Pure Tension)']

        if 'RC' in loc:  [x, y] = [R.Mn[n], R.Pn[n]]
        if 'FRP' in loc: [x, y] = [F.Mn[n], F.Pn[n]]
...
def Column(In, R, F, loc, selected_row):
        ####! 공통
        fig = go.Figure();  f = 1.06;  fig_width = 550;  fig_height = f*fig_width

        # 그림 상자 외부 박스    
        xlim = 500;  ylim = f*xlim
        shape(fig, 'rect', 0,0,xlim,ylim, 'white', 'purple', 1)        

        # 공통 기본 치수
        base_scale = 0.5*xlim
        if 'Rectangle' in In.Section_Type:
            mr = base_scale/max([In.be, In.height]);  w = In.height*mr;  h = In.be*mr
            typ = 'rect';  txt = f'h = {In.hD:,.0f} mm'
        ...
        for L in range(In.Layer):
            if L == 0: color = 'rgba(255,0,0, 0.2)'
            if L == 1: color = 'rgba(0,255,0, 0.2)'
            if L == 2: color = 'rgba(0,0,255, 0.2)'
            dc = In.dc[L]*mr;  dia = In.dia[L]*mr;  cr = 1.2*dia/2;  gap = dc - cr    # 원래 반경(cr)보다 20% 크게~ 보이게
        ...
        if 'Common' in loc:            
            ...
            if selected_row == None:   # 초기값 (startupFcn)
                ...
                ee = 0.25*In.hD;  xee = x + ee*mr;  Pd = 0;  ep_t = 0;  fc = 0;  ft = 0
                if 'RC' in loc:   ep_t = -0.002  # 인장(-)
                if 'FRP' in loc:  ep_t = -0.004
                ...
                if 'E ' in selected_row:
                    if 'RC' in loc:  [n, txt] = [4, 'E (ε<sub>t</sub> = 2.5ε<sub>y</sub>)']
                    if 'FRP' in loc: [n, txt] = [4, 'E (ε<sub>t</sub> = 0.8ε<sub>fu</sub>)']
                if 'F (P' in selected_row: [n, txt] = [5, 'F (Pure Moment)']
                if 'G (P' in selected_row: [n, txt] = [6, 'G (Pure Tension)']

                if 'RC'  in loc:  [ee, Pd, cc, ep_t, fc, ft] = [R.e[n], R.Pd[n], R.c[n], R.ep_s[n,1], R.fs[n,0], R.fs[n,1]]
                if 'FRP' in loc:  [ee, Pd, cc, ep_t, fc, ft] = [F.e[n], F.Pd[n], F.c[n], F.ep_s[n,1], F.fs[n,0], F.fs[n,1]]
                txtPn = f'{Pd:,.1f} kN'
                if ee == np.inf: txte = 'inf'
                elif ee == 0:    txte = ''
                else:            txte = f'{ee:,.1f} mm'
            
            y_col2 = y_col + 70;  y_ep = 0.6*ylim
            ...
            yee = y_col+(y_col2-y_col)/1.8;  arrow_size = 18*1.2;  arrow_width = arrow_size/3  # 화살표 두께 고정 !!!
            ...
            a0 = xee;  a1 = a0 - arrow_width/2; a2 = a0 + arrow_width/2
            b1 = b0 + sgn*arrow_size;   b2 = b1
            fig.add_shape(    # 화살표
                type="path", path=f"M {a0} {b0} L {a1} {b1} L {a2} {b2} Z", # M = move to, L = line to, Z = close path
            ...
            ### 기둥 변형률, 응력
            fix = R.ep_cu;  y_ep_cu = 80;  xd = x + w - In.depth*mr
            xcc = x + w - cc*mr if cc <= In.depth else xd
            y_ep_c = y_ep - y_ep_cu*ep_cu/fix
            y_ep_t = y_ep - y_ep_cu*ep_t/fix
            
            if selected_row != None:
                if 'RC'  in loc:  phi = R.phi[n];  phi_Status = R.phi_Status[n]
                if 'FRP' in loc:  phi = F.phi[n];  phi_Status = F.phi_Status[n]
                color = 'black'
                if 'Ten' in phi_Status:  color = 'red'
                if 'Com' in phi_Status:  color = 'blue'
            ...
            if selected_row == None:  txt = 'ε<sub>t</sub>'
            ...
            a0 = xcc;  a1 = x + w;  a2 = a1;      a3 = a2
            b0 = y_ep; b1 = b0;     b2 = y_ep_c;  b3 = b2
            if cc > In.depth:  [a3, b3] = [xcc, y_ep_t]
            fig.add_shape(    # 압축 변형률 삼각형, 사각형 음영
                type="path", path=f"M {a0} {b0} L {a1} {b1} L {a2} {b2} L {a3} {b3} Z", # M = move to, L = line to, Z = close path
                fillcolor=shadecolor, line=dict(color='black', width=2, ), )

            shadecolor = 'rgba(255,0,0, 0.3)'
            a0 = xcc;  a1 = x + dc;  a2 = a1;     a3 = a2
            b0 = y_ep; b1 = b0;      b2 = y_ep_t; b3 = b2
            if cc == -np.inf:
                a0 = xd;  a1 = a0;  a2 = x + w;  a3 = a2
                b1 = y_ep_t; b2 = b1;  b3 = b0
            fig.add_shape(    # 인장 변형률 삼각형 음영
                type="path", path=f"M {a0} {b0} L {a1} {b1} L {a2} {b2} L {a3} {b3} Z", # M = move to, L = line to, Z = close path
                fillcolor=shadecolor, line=dict(color='black', width=2, ), )            
            ### 기둥 변형률
            ###! 기둥 Pn, e, c & 변형률
        ...
        fig.update_xaxes(range=[0, xlim]);  fig.update_yaxes(range=[0, ylim])
